...
inotifywait -m -q -e close_write,moved_to --format '%w%f' "$(dirname "$SETTINGS_FILE")" "$(dirname "$ENV_FILE")" | while read -r filepath; do
    ...
    if [[ "$filepath" == "$SETTINGS_FILE" ]]; then
        ...
        NEW_STARTUP=$(jq -c '.startup' "$SETTINGS_FILE" 2>/dev/null)
...
EOF
        ...
        # 4. Update Startup entries if changed
        if [[ "$NEW_STARTUP" != "$(cat "$CACHE_DIR/startup" 2>/dev/null)" && "$NEW_STARTUP" != "null" && -n "$NEW_STARTUP" ]]; then
            echo "Startup entries changed. Regenerating..."
            cat << 'EOF' > "$AUTOSTART_CONF"
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  ◈ AUTOSTART (Auto-generated by Quickshell Settings Watcher)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

EOF
            jq -r '.startup[]? | "exec-once = \(.command)"' "$SETTINGS_FILE" >> "$AUTOSTART_CONF"
            GUIDE=$(jq -r '.openGuideAtStartup // false' "$SETTINGS_FILE" 2>/dev/null)
            if [[ "$GUIDE" == "true" ]]; then
                echo "exec-once = ~/.config/hypr/scripts/qs_manager.sh toggle guide &" >> "$AUTOSTART_CONF"
            else
                echo "# exec-once = ~/.config/hypr/scripts/qs_manager.sh toggle guide &" >> "$AUTOSTART_CONF"
            fi
